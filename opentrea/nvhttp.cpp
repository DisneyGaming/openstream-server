...
void update_id_client(const std::string &uniqueID, std::string &&cert, op_e op) {
  ...
  if(!config::opentrea.flags[config::flag::FRESH_STATE]) {
    save_state();
  }
}
...
void pair(std::shared_ptr<safe::queue_t<crypto::x509_t>> &add_cert, std::shared_ptr<typename SimpleWeb::ServerBase<T>::Response> response, std::shared_ptr<typename SimpleWeb::ServerBase<T>::Request> request) {
  ...
  if(it = args.find("phrase"); it != std::end(args)) {
    if(it->second == "getservercert"sv) {
      ...
      if(config::opentrea.flags[config::flag::PIN_STDIN]) {
        std::string pin;
...
void serverinfo(std::shared_ptr<typename SimpleWeb::ServerBase<T>::Response> response, std::shared_ptr<typename SimpleWeb::ServerBase<T>::Request> request) {
  ...
  tree.put("root.<xmlattr>.status_code", 200);
  tree.put("root.hostname", config::nvhttp.opentrea_name);
...
void appasset(resp_https_t response, req_https_t request) {
  ...
  std::ifstream in(OPENTREA_ASSETS_DIR "/box.png");
...
int create_creds(const std::string &pkey, const std::string &cert) {
  ...
  auto creds = crypto::gen_creds("Opentrea Gamestream Host"sv, 2048);
...
void start(std::shared_ptr<safe::signal_t> shutdown_event) {
  bool clean_slate = config::opentrea.flags[config::flag::FRESH_STATE];
  if(clean_slate) {
    unique_id = util::uuid_t::generate().string();
