...
#include "opentrea/platform/common.h"
#include "opentrea/main.h"
...
public:
  void clear_mouse() {
    std::filesystem::path mouse_path { "opentrea_mouse"sv };
  ...
  int create_mouse() {
    ...
    if(err) {
      BOOST_LOG(error) << "Could not create Opentrea Mouse: "sv << strerror(-err);
      return -1;
    }

    std::filesystem::create_symlink(libevdev_uinput_get_devnode(mouse_input.get()), "opentrea_mouse"sv);
  ...
  int alloc_gamepad(int nr) {
    ...
    if(err) {
      BOOST_LOG(error) << "Could not create Opentrea Gamepad: "sv << strerror(-err);
      return -1;
    }
...
evdev_t mouse() {
  ...
  libevdev_set_uniq(dev.get(), "Opentrea Mouse");
...
evdev_t x360() {
  ...
  libevdev_set_uniq(dev.get(), "Opentrea Gamepad");
