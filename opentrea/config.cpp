...
#define APPS_JSON_PATH OPENTREA_ASSETS_DIR "/" APPS_JSON
namespace config {
...
nvhttp_t nvhttp {
  ...
  boost::asio::ip::host_name(), // opentrea_name,
  "opentrea_state.json"s // file_state
};
...
int apply_flags(const char *line) {
  ...
  while(*line != '\0') {
    switch(*line) {
      case '0':
        config::opentrea.flags[config::flag::PIN_STDIN].flip();
        break;
      case '1':
        config::opentrea.flags[config::flag::FRESH_STATE].flip();
        break;
      default:
...
void apply_config(std::unordered_map<std::string, std::string> &&vars) {
  ...
  string_f(vars, "opentrea_name", nvhttp.opentrea_name);
...
int parse(int argc, char *argv[]) {
  const char *config_file = OPENTREA_ASSETS_DIR "/opentrea.conf";
