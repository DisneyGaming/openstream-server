...
nvhttp_t nvhttp {
  ...
  boost::asio::ip::host_name(), // openstream_name,
  "openstream_state.json"s // file_state
};
...
int apply_flags(const char *line) {
  ...
  while(*line != '\0') {
    switch(*line) {
      case '0':
        config::openstream.flags[config::flag::PIN_STDIN].flip();
        break;
      case '1':
        config::openstream.flags[config::flag::FRESH_STATE].flip();
        break;
      default:
...
void apply_config(std::unordered_map<std::string, std::string> &&vars) {
  ...
  string_f(vars, "openstream_name", nvhttp.openstream_name);
  ...
  if(!log_level_string.empty()) {
    if(log_level_string == "verbose"sv) {
      openstream.min_log_level = 0;
    }
    else if(log_level_string == "debug"sv) {
      openstream.min_log_level = 1;
    }
    else if(log_level_string == "info"sv) {
      openstream.min_log_level = 2;
    }
    else if(log_level_string == "warning"sv) {
      openstream.min_log_level = 3;
    }
    else if(log_level_string == "error"sv) {
      openstream.min_log_level = 4;
    }
    else if(log_level_string == "fatal"sv) {
      openstream.min_log_level = 5;
    }
    else if(log_level_string == "none"sv) {
      openstream.min_log_level = 6;
    }
  }
...
int parse(int argc, char *argv[]) {
  const char *config_file = OPENSTREAM_ASSETS_DIR "/openstream.conf";
